import React from 'react'
import Cropper from 'cropperjs'

import 'cropperjs/dist/cropper.css'
import './styles.css'

class CropperJS extends React.Component {
	constructor(props) {
		super(props)

		this.cropElement = React.createRef()
		this.state = { src: null, imageLoaded: false }
	}

	enable = () => {
		this.cropper.enable()
		this.setState({
			disabled: false
		})
	}

	disable = () => {
		this.setState({
			disabled: true
		})
	}

	zoom = value => {
		try {
			this.cropper.zoomTo(value / 50)
		}
		catch(err) {}
	}

	rotateCCW = () => {
		this.cropper.rotate(-90)
	}

	rotateCW = () => {
		this.cropper.rotate(90)
	}

	getCroppedImage = () => {
		return this.cropper.getCroppedCanvas()
	}

	load = (src) => {
		if (this.cropper) {
			this.cropper.destroy()
		}

		this.setState({
			src,
			imageLoaded: false
		})
	}

	destroy = () => {
		this.setState({
			src: null
		})
		this.cropper.destroy()
	}

	imageLoaded = () => {
		this.cropElement.current.addEventListener('ready', () => {
			this.cropper.zoomTo(1)
			this.setState({
				imageLoaded: true
			})
		})

		this.cropper = new Cropper(this.cropElement.current, {
			movable: true,
			rotatable: true,
			scalable: true,
			checkOrientation: true,
			zoomable: true,
			zoomOnTouch: false,
			zoomOnWheel: false,
			viewMode: 1,
			guides: false,
			strict: false,
			minContainerWidth: 560,
			minContainerHeight: 400
		})
	}

	render() {
		const { src, imageLoaded, disabled } = this.state

		return (
			<div className={disabled ? "cropolis-disabled" : ""} style={{ position: 'relative' }}>
				{ !imageLoaded &&
					<svg className="spinner" version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="24px" height="30px" viewBox="0 0 24 30">
						<rect x="0" y="7.5433" width="4" height="14.9134" fill="#fff" opacity="0.2">
						<animate attributeName="opacity" attributeType="XML" values="0.2; 1; .2" begin="0s" dur="0.6s" repeatCount="indefinite"></animate>
						<animate attributeName="height" attributeType="XML" values="10; 20; 10" begin="0s" dur="0.6s" repeatCount="indefinite"></animate>
						<animate attributeName="y" attributeType="XML" values="10; 5; 10" begin="0s" dur="0.6s" repeatCount="indefinite"></animate>
						</rect>
						<rect x="8" y="9.9567" width="4" height="10.0866" fill="#fff" opacity="0.2">
						<animate attributeName="opacity" attributeType="XML" values="0.2; 1; .2" begin="0.15s" dur="0.6s" repeatCount="indefinite"></animate>
						<animate attributeName="height" attributeType="XML" values="10; 20; 10" begin="0.15s" dur="0.6s" repeatCount="indefinite"></animate>
						<animate attributeName="y" attributeType="XML" values="10; 5; 10" begin="0.15s" dur="0.6s" repeatCount="indefinite"></animate>
						</rect>
						<rect x="16" y="7.4567" width="4" height="15.0866" fill="#fff" opacity="0.2">
						<animate attributeName="opacity" attributeType="XML" values="0.2; 1; .2" begin="0.3s" dur="0.6s" repeatCount="indefinite"></animate>
						<animate attributeName="height" attributeType="XML" values="10; 20; 10" begin="0.3s" dur="0.6s" repeatCount="indefinite"></animate>
						<animate attributeName="y" attributeType="XML" values="10; 5; 10" begin="0.3s" dur="0.6s" repeatCount="indefinite"></animate>
						</rect>
					</svg>
				}
				<div style={{ display: imageLoaded === true ? 'block' : 'none' }}>
					<img id="cropper" ref={this.cropElement} src={src} style={{ maxWidth: '500px' }} onLoad={this.imageLoaded} />
				</div>
				{ disabled && <div className="cropolis-disabled-overlay"></div> }
			</div>
		)
	}
}

export default CropperJS
